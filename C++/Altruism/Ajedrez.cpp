...
string leerFEN(string posicion){
    ...
    const char *cstr = posicion.c_str();
    ifstream input;
	input.open(cstr, ios::in);
    getline(input, texto);

    return texto;
}

void imprimirTablero(string posiciones[8][8]){
...
void guardarPosicion(string posiciones[8][8]){
...
boolean verificarMovimiento(string pieza, string posicionInicial, string posicionFinal, string posiciones[8][8]){
    ...
    if (pieza == "q" || pieza == "Q"){
        ...
        boolean movimientoTorre = verificarMovimiento(piezaTorre, posicionInicial, posicionFinal, posiciones);
        boolean movimientoAlfil = verificarMovimiento(piezaAlfil, posicionInicial, posicionFinal, posiciones);
...
boolean verificarMovimientoAlgebraico(string posicionAlgInicial, string posicionAlgFinal, string posiciones[8][8]){
    ...
    return verificarMovimiento(piezaInicial, posicionInicial, posicionFinal, posiciones);
}

void calcularMovimiento(string pieza, string posiciones[8][8]){
    ...
    for(int i = 0; i < posicion_piezas.size(); i++){
            ...
            for(int i = 1; i <= 8; i++){
                for(int j = 1; j <= 8; j++){
                    ...
                    if(verificarMovimiento(pieza, posicionInicial, posicionFinal, posiciones)){
...
void realizarMovimiento(string posicionAlgInicial, string posicionAlgFinal, string posiciones[8][8]){

    if(verificarMovimientoAlgebraico(posicionAlgInicial, posicionAlgFinal, posiciones)){
...
int main(){
    ...
    string posicion = "fen_ejemplo.fen";
    ...
    guardarPosicion(posiciones);
    // Calcula el movimiento de los peones blancos
    calcularMovimiento("P", posiciones);
    imprimirTablero(posiciones);
